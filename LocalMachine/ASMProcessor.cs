...
namespace LocalMachine
{
	public class ASMProcessor
	{
		...
		public int PlaceMatrix(Matrix matrix){
			lock(this)
			{
				for(int a=0;a<currentcode.matrixmemory.Length;a++)
					if(Object.ReferenceEquals(currentcode.matrixmemory[a], null))
					{
		...
		private void ASM_Add(ref ProgramThread currentthread)
		{
			...
			else if((tempvar2type==MEMTYPECONSTANTS.REFERENCE || tempvar2type==MEMTYPECONSTANTS.TEMPREFERENCE) && (tempvar1type==MEMTYPECONSTANTS.REFERENCE || tempvar1type==MEMTYPECONSTANTS.TEMPREFERENCE))
			{
				...
				if(tempvar1type==MEMTYPECONSTANTS.TEMPREFERENCE)
				{
					...
					if(tempvar2type==MEMTYPECONSTANTS.TEMPREFERENCE)currentcode.matrixmemory[(int)tempvar2]= null;
				}
		...
		private void ASM_Store(ref ProgramThread currentthread){
			...
			if(rmm>=0)
				currentcode.matrixmemory[rmm]=null;
		...
		private void ASM_SetMatrixToDevice(ref ProgramThread currentthread)
		{
			...
			if(tempvar2type == MEMTYPECONSTANTS.TEMPREFERENCE)
				currentcode.matrixmemory[tempvar2]= null;
		}
		private void ASM_IsMatrix(ref ProgramThread currentthread)
		{
			...
			if(tempvar1type == MEMTYPECONSTANTS.TEMPREFERENCE)
				currentcode.matrixmemory[tempvar1]= null;
		}
	...
	/*	private void ASM_ParallelStart(ref ProgramThread currentthread){
			...
			ushort pcounter=1,scounter=0;

			while(pcounter>0){
				currentthread.inspointer++;
				
				switch((ASMPROCESSOR_OPERATIONS)currentcode.instructions[currentthread.inspointer]){
				
					case ASMPROCESSOR_OPERATIONS.PARALLELSTART:
						pcounter++;
						break;
					case ASMPROCESSOR_OPERATIONS.PARALLELEND:
						pcounter--;
						if(scounter==0)scounter=1;
						break;
					case ASMPROCESSOR_OPERATIONS.STATEMENTSTART:
						if(scounter==0)
							tempthread.mthread.inspointer=currentthread.inspointer+1;
						scounter++;
						break;
					case ASMPROCESSOR_OPERATIONS.STATEMENTEND:
						scounter--;
						break;
				}
				
				if(scounter==0){
					tempthread.mthread.endthread = currentthread.inspointer-1;
					if(tempthread.mthread.endthread>tempthread.mthread.inspointer){
		...
		private void ASM_ParallelStart(ref ProgramThread currentthread)
		{
			...
			WrappedThreadObject tempthread = new WrappedThreadObject(this);
			ArrayList threads= new ArrayList();
			ArrayList remotethreads = new ArrayList();

			while(currentcode.instructions[currentthread.inspointer]==(ushort)ASMPROCESSOR_OPERATIONS.CREATETHREAD)
			{
				if(RemoteProcessorsManager.FreeProcessors.Count>0)
				{
					RemoteProcessor tmprp = RemoteProcessorsManager.PopProcessor();
					currentthread.inspointer++;
					tmprp.mthread.inspointer = paralleloffset + (int)currentcode.numconstants[
						currentcode.instructions[currentthread.inspointer]];
					currentthread.inspointer++;
					tmprp.mthread.endthread = paralleloffset + (int)currentcode.numconstants[
						currentcode.instructions[currentthread.inspointer]];
					currentthread.inspointer++;
					tmprp.localprocessor = this;
					tmprp.memstart = currentthread.inspointer+1;
					tmprp.memend = currentthread.inspointer+currentcode.instructions[currentthread.inspointer];
					currentthread.inspointer += (currentcode.instructions[currentthread.inspointer]+1);
					remotethreads.Add(tmprp);
					tmprp.Proccess();
				}
				else{
					currentthread.inspointer++;
			...
			foreach(WrappedThreadObject temp in threads)
				temp.thread.Join();
			foreach(RemoteProcessor tmprp in remotethreads)
			{
				tmprp.Join();
				RemoteProcessorsManager.FreeProcessor(tmprp);
			}
	...
	public class WrappedThreadObject{
		
		public ProgramThread mthread;
		ASMProcessor threadprocessor;
